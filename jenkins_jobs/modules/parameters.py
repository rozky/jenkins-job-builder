...
def extended_choice_param(parser, xml_parent, data):
    pdef = XML.SubElement(xml_parent, 'com.cwctravel.hudson.plugins.extended__choice__parameter.ExtendedChoiceParameterDefinition', 
    ...
    XML.SubElement(pdef, 'visibleItemCount').text = data.get('visibleItemCount', '5')
    XML.SubElement(pdef, 'multiSelectDelimiter').text = data.get('multiSelectDelimiter', ',')
